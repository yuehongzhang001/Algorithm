...
public class LinkedPositionalList<E> implements PositionalList<E> {
    ...
    public String toString(){
        String listContent="[";//start with "["
        Position<E> cursor = first();//use a cursor to point to first position.
        listContent+=cursor.getElement();//get the element cursor points to.
        for(int i=1;i<size;i++){//loop through the other Nodes
            cursor = after(cursor);//set cursor point to next Node
            listContent += " " + cursor.getElement();//get the contents in the node and add to string
        }
        listContent+="]";//end of the string
        return listContent;//the list contents
    }
}
